           ...
           {this.props.posts.map((post, i) => {
                ...
                <hr id='postlines' />
                    ...
                    </div>
                    {
                        ...
                        <button class='border border-secondary btn btn-secondary-outline btn-sm' onClick={() => this.props.showEditForm(post)}><img width='15px' alt='edit button' src='https://i.imgur.com/Wie5RHb.png' /></button> 
                    }
                    {
                        post.author.id === this.props.currentUserId && 
                        <button type='submit' class='border border-secondary btn btn-secondary-outline btn-sm' onClick={() => this.props.delete(post.id)}><img width='15px' alt='delete button' src='https://i.imgur.com/1uWeVvr.png' /></button> 
                    }
                    {   this.props.modal &&  
                        <div id='modalbg'>
                            <div id='modalmain'>
                        ...
                        this.props.comments.map((comment, i) => {
                            if (post.id === comment.postid.id){
                                    ...
                                    <span id='author'>Post by: {comment.author.username} on {comment.date}</span>
                                {
                                    ...
                                    <button class='border border-secondary btn btn-secondary-outline btn-sm' onClick={() => this.props.commentEdit(comment)}><img width='13px' alt='edit button' src='https://i.imgur.com/Wie5RHb.png' /></button>
                                }
                                {
                                    (comment.author.id === this.props.currentUserId || post.author.id === this.props.currentUserId) && 
                                    <button class='border border-secondary btn btn-secondary-outline btn-sm' type='submit' onClick={() => this.props.deleteComment(comment.id)}><img width='13px' alt='delete button' src='https://i.imgur.com/1uWeVvr.png' /></button> 
                                }
